...
public class Create : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < numSphere; i++)
        {
            ...
            initPos[i] = new Vector3(r * Mathf.Sqrt(2f) * (Mathf.Pow(Mathf.Sin(i * 2f * Mathf.PI / (float)numSphere), 3f)),
                r * (Mathf.Pow(-Mathf.Cos(i * 2 * Mathf.PI / numSphere), 3f) - Mathf.Pow(Mathf.Cos(i * 2 * Mathf.PI / numSphere), 2f) + (2 * Mathf.Cos(i * 2 * Mathf.PI / numSphere))), 10f);
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numSphere; i++)
        {
            ...
            Color color = Color.red;
                //HSVToRGB(Mathf.Abs(hue * Mathf.Sin(time)), Mathf.Cos(time), 2f + Mathf.Cos(time)); // Full saturation and brightness
            sphereRenderer.material.color = color;
        }
    }
}